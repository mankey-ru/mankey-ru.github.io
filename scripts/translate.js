...
async function main() {
  try {
    ...
    if (!provider) {
      console.error('ERROR: No API key set. Set OPENROUTER_API_KEY (preferred) or OPENAI_API_KEY as repository secrets.');
      process.exit(1);
    }
    ...
    if (!content.startsWith('<')) {
      console.warn('Warning: translated content does not start with "<"; writing output anyway.');
    }
